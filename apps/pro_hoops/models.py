...
class ScheduleManager(models.Manager):
    def get_or_create(self, api_id_key,home_team,away_team, match_time):
        try:
            return ScheduleNBA.objects.get(api_id_key=api_id_key.lower())
        except:
            return ScheduleNBA.objects.create(api_id_key.lower(),away_team,home_team,match_time)

class ScheduleNBA(models.Model):
...
class OddsNBA(models.Model):
    draw_line = models.IntegerField(null=True)
    entity_id = models.CharField(max_length = 255)
    schedule = models.ForeignKey(ScheduleNBA, related_name = 'odds', null= True, blank=True,  on_delete = models.CASCADE)
