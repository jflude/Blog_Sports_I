...
#######This for loop filters games so games displayed in html are unique and not repeated####
    ...
    return render(request, 'baseball/baseball_index.html', context)


def show_baseball_odds(request,id):
    game=ScheduleMLB.objects.get(id=id)
    game_odds=OddsMLB.objects.filter(schedule_id= game.id)
    inning_odds = OddsMLB.objects.filter()
    context = {
        'games': game,
        'game_odds': game_odds,
    ...
    return render(request, 'baseball/baseball_odds.html', context)
