...
import django
from apps.pro_hoops.models import *
import sqlite3
...
def print_sport(sport):
    for schedule in get_schedule(sport):
        schedule_nba = ScheduleNBA()
        schedule_nba.api_id_key = schedule['ID']
        schedule_nba.away_rot = int(schedule['AwayROT'])
        schedule_nba.away_team = schedule['AwayTeam']
        schedule_nba.home_rot = int(schedule['HomeROT'])
        schedule_nba.home_team = schedule['HomeTeam']
        schedule_nba.match_time = schedule['MatchTime']
        schedule_nba.save()
        for odds in schedule['Odds']:
            ...
            for item in odds_list:
                ...
                for event_odd in item['Odds']:
                    ...
                    game_odd.schedule = schedule_nba
                    game_odd.save()

                print(":-)" *80)

print_sport('NBA')
#pprint(get_schedule("NBA"))
